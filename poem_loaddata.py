...
# (base_path,20,w2v.model,'train')
# DataLoader(base_path,batch_size,num_unrollings,w2v.model,'train')
class DataLoader():
    def __init__(self, data_dir, batch_size, seq_max_length, w2v, data_type, train_frac=0.9, valid_frac=0.05):
    ...
    def create_batches(self, samples):
        ...
        x_lengths = []
        print(new_sample_size, self.seq_max_length)
        ...
        y_seqs[:, :-1] = x_seqs[:, 1:]  # x的1-最后给，y的0-倒数第二个
        y_seqs[:, -1] = x_seqs[:, 0]  # x的第0个给y的最后一个
...
if __name__ == '__main__':
    ...
    dataloader = DataLoader(base_path, 20, 64, w2v.model, 'train')
