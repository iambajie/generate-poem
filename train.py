...
from poem_charrnn import CharRNNLM
from poem_config import config_poem_train
from poem_word2vec import Word2Vec
from poem_loaddata import DataLoader
...
def main(args=''):
    ...
    if len(args.init_dir) != 0:
        ...
        if os.path.exists(args.output_dir):
            shutil.rmtree(args.output_dir)#递归删除文件夹下的所有子文件夹和子文件
        for paths in [args.save_model, args.save_best_model, args.tb_log_dir]:#创建
    ...
    train_data_loader = DataLoader(base_path,batch_size,num_unrollings,w2v.model,'train',train_frac=train_frac)
    test1_data_loader = DataLoader(base_path,batch_size,num_unrollings,w2v.model,'test',train_frac=valid_frac)
